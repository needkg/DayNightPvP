...
public class UpdateService {
    ...
    public void checkUpdate(PlayerJoinEvent event) {
        try {
            ...
            event.getPlayer().sendMessage(messageManager.getMessage(Message.SYSTEM_UPDATE_CHECK_FAILED));
        }
    }
    ...
    private void notifyUpdateAvailable(Player player, String latestVersion) {
        ...
        player.sendMessage(messageManager.getMessage(Message.SYSTEM_UPDATE_AVAILABLE));
        player.sendMessage(messageManager.getMessage(Message.SYSTEM_UPDATE_CURRENT_VERSION).replace("{0}", currentPluginVersion));
        player.sendMessage(messageManager.getMessage(Message.SYSTEM_UPDATE_LATEST_VERSION).replace("{0}", latestVersion));
        player.spigot().sendMessage(updateLink);
    }

    private TextComponent createUpdateLink() {
        TextComponent link = new TextComponent(messageManager.getMessage(Message.SYSTEM_UPDATE_CLICK_TO_UPDATE));
